...
Perche' questo semplifica davvero, e non e' una scorciatoia: `omega` e'
l'invariante cinematico. In assenza di gradienti di corrente la frequenza
assoluta si conserva nel trasporto verso riva (eq. 2.33), quindi
`omega` alla boa = `omega` nella scena **qualunque sia la profondita'**.
Non serve ne' un DEM ne' che boa e scena stiano alla stessa quota.

L'unica eccezione e' la corrente: `omega = sigma + k.U`. Un estuario con
maree di 1-1.5 m/s sposta `omega` di una quantita' che non sappiamo separare.
Per questo la coda e' ordinata per **rischio di corrente**, non per vicinanza
della boa.

GATE OFFLINE gia' applicati in BLOCK26_QUERY_QUEUE.csv:
...
Uso:  python run_block26_frequency_query.py BLOCK26_QUERY_QUEUE.csv <outdir>
"""
...
def main():
    ...
    with open(outdir / "BLOCK26_FREQUENCY_GATE.csv", "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=keys); w.writeheader()
        for r in res:
            w.writerow(r)
    cands = [r for r in res if r["verdict"] == "CANDIDATE"]
    cands.sort(key=lambda r: (r["axial_difference_deg"], -r["steepness_H_over_L"]))
    json.dump({"n_queue": len(rows), "n_candidates": len(cands),
               ...
               "candidates": cands},
              open(outdir / "BLOCK26_SUMMARY.json", "w"), indent=2, default=str)
