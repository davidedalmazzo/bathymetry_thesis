...
def sources(root):
    ...
    files+=['code/frf_client_cli.py','code/run_block32_frf_client.py','code/run_block30_frf_conditions.py','code/run_block33_frf_verification.py','code/run_block34_frf_operational.py',
            'tests/test_frf_client.py','tests/test_frf_block33.py','tests/test_frf_operational.py','tests/test_block30_frf_conditions.py','tests/conftest.py','requirements-thesis.txt','pytest.ini','code/README_FRF_CLIENT.md','FRF_QUICKSTART.md']
...
def delivery_manifest(root,base):
    ...
    for name in listed:
        ...
        rows.append({'path':name,'bytes':(root/name).stat().st_size,'sha256':digest((root/name).read_bytes()),'tracked':name in tracked,'modified_or_new':name not in tracked or name in modified,'gitignored':ignored})
    save_json(base/'FILES_TO_VERSION.json',{'required_sources':rows,'new_block_directory':base.relative_to(root).as_posix(),'additional_modified_docs':['README.md','WORKLOG.md','.gitignore','CHECKPOINT_34.md'],
    ...
    extras=[root/'CHECKPOINT_34.md',root/'README.md',root/'WORKLOG.md',root/'.gitignore']
    ...
    artifacts=[p for p in base.rglob('*') if p.is_file() and not any(x in ('cache','network') or x.startswith('delivery_copy') for x in p.parts) and p.name!='MANIFEST.json']
    save_json(base/'MANIFEST.json',{'named_tranche':state,'root_relative_paths':True,'artifacts':[{'path':p.relative_to(root).as_posix(),'bytes':p.stat().st_size,'sha256':digest(p.read_bytes())} for p in sorted(set(artifacts+[root/n for n in listed]+[p for p in extras if p.exists()]))]})
