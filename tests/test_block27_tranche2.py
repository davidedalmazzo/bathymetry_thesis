...
OUT=ROOT/'umbra/selezione_scene/Block27_frequency_query/representativity_v2'
...
def test_new_subset_timestamp_hash_masks_and_offline_reparse():
    ...
    raw=resolve_historical(r['payload_path'], ROOT); assert m.sha(raw)==r['payload_sha256']
...
def test_local_42084_preserved_and_not_relabelled_42094():
    ...
    assert all(m.sha(resolve_historical(r['payload_path'], ROOT))==r['payload_sha256'] for r in old)
