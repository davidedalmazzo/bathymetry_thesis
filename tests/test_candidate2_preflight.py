...
def test_config_is_single_candidate_and_frozen_band():
 c=json.loads((root()/'umbra/selezione_scene/Block23_candidate2_metadata_preflight/BLOCK23_CONFIG.json').read_text())
...
def test_declared_assets_do_not_equal_verified_availability():
 with (root()/'umbra/selezione_scene/Block23_candidate2_metadata_preflight/BLOCK23_ASSET_AVAILABILITY.csv').open() as f:r={x["asset"]:x for x in csv.DictReader(f)}
...
def test_vendor_collect_identity_and_polarization():
 x=json.loads((root()/'umbra/selezione_scene/Block23_candidate2_metadata_preflight/remote_raw/candidate2_metadata.json').read_text())["collects"][0]
...
def test_range_check_cannot_be_promoted_from_missing_sicd():
 x=json.loads((root()/'umbra/selezione_scene/Block23_candidate2_metadata_preflight/BLOCK23_WAVE_RANGE_GEOMETRY.json').read_text())
