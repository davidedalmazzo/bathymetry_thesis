...
Run from the thesis repository root. The current in-repository layout migration
must be versioned separately from the previously published Block33/34 sources. No credentials needed
...
Block34 execution evidence and actual named budget are in docs/checkpoints/CHECKPOINT_34.md and
duck_frf/Block34_frf_operational; generic examples do not authorize another live run.
